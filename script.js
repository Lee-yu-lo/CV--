(() => {
  ...
  // 数据驱动渲染：页面文案全部来自 index.html 顶部 JSON。
  ...
  function renderData() {
    ...
    const sectionIds = ['about', 'responsibilities', 'tools', 'method', 'contact'];
    const links = data.nav.map((label, index) => `<a href="#${sectionIds[index] || 'top'}">${escapeHTML(label)}</a>`).join('');
    ...
    $('[data-responsibilities]').innerHTML = data.responsibilities.items.map(item => `<article class="responsibility-card"><div class="card-num">${escapeHTML(item.number)}</div><div><div class="card-title">${escapeHTML(item.title)}</div><div class="card-subtitle">${escapeHTML(item.subtitle)}</div></div><div class="card-body"><span class="card-tag">${escapeHTML(item.tag)}</span>${escapeHTML(item.body)}</div></article>`).join('');
    ...
    $('[data-email]').href = `mailto:${encodeURIComponent(email)}`;
    $('[data-email-text]').textContent = email;
    $('.menu-toggle').setAttribute('aria-label', data.ui.menuOpen);
  }
  ...
  function initMenu() {
    ...
    const toggle = () => {
      ...
      button.setAttribute('aria-label', open ? data.ui.menuClose : data.ui.menuOpen);
